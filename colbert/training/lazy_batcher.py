...
from colbert.modeling.tokenization import QueryTokenizer, DocTokenizer, tensorize_triples, QueryTokenizer_X, DocTokenizer_X
...
class LazyBatcher():
    def __init__(self, args, rank=0, nranks=1):
        ...
        if args.use_roberta:
            print("tokenizer:tobertaを使用")
            self.query_tokenizer = QueryTokenizer_X(args.query_maxlen,args.model_name)
            self.doc_tokenizer = DocTokenizer_X(args.doc_maxlen,args.model_name)
        else:
    ...
    def __next__(self):
        ...
        for position in range(offset, endpos):
            query, pos, neg = map(int,self.triples[position])
