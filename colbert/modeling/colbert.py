...
class ColBERT_X(RobertaPreTrainedModel):
    config_class = XLMRobertaConfig

    def __init__(self,config,tokenizer, query_maxlen, doc_maxlen, mask_punctuation, dim=128, similarity_metric='cosine'):

        super(ColBERT_X, self).__init__(config)
        ...
        if self.mask_punctuation:
            self.tokenizer = XLMRobertaTokenizer.from_pretrained(tokenizer)
            self.tokenizer.add_tokens(['[unused1]'])#Q
            self.tokenizer.add_tokens(['[unused2]'])#D
            self.tokenizer.add_tokens(['[zho]'])
            self.tokenizer.add_tokens(['[fas]'])
            self.tokenizer.add_tokens(['[rus]'])
