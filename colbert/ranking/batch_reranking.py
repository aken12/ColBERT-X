...
def batch_rerank(args):
    ...
    if args.doc_id:
        id_list = []
        with open(args.doc_id,'r')as f_id:
            for line in f_id:
                line = line.split('\t')
        ...
        with torch.no_grad():
            for query_index, qid in enumerate(queries):
                ...
                if args.doc_id:
                    ...
                    scores_topk = torch.sort(torch.tensor(scores),descending=True)
                    #pids = torch.tensor(scores_topk.indices).tolist()
                    scores_values = scores_topk.values.tolist()
                    scores_indices = scores_topk.indices.tolist()
                    #print(scores)
                    ...
                    for rank,idx in enumerate(scores_indices):
                        ...
                        if docid not in dids:
                            dids.append(docid)
                            d_scores.append(scores_values[rank])
                        else:
                            print("被り")
                        if len(dids) == 1000:
                            break
                    ranking = [(score, did, None) for did, score in zip(dids, d_scores)]
                    assert len(ranking) <= MAX_DEPTH_LOGGED, (len(ranking), MAX_DEPTH_LOGGED)

                    rlogger.log(qid, ranking, is_ranked=True, print_positions=[1, 2] if query_index % 100 == 0 else [],doc_id=args.doc_id)
                else:
                    scores_topk = torch.tensor(scores).topk(K, largest=True, sorted=True)
