...
class Ranker():
    def __init__(self, args, inference, faiss_depth=1024):
        ...
        if args.use_roberta:
            self.index = IndexPart(args.index_path, dim=inference.roberta.dim, part_range=args.part_range, verbose=True)
        else:
    ...
    def rank(self, Q, pids=None):
        pids = self.retrieve(Q, verbose=False)[0] if pids is None else pids ##クエリに近いpids depth分だけ
