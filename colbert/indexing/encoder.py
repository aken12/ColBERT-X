...
class CollectionEncoder():
    ...
    def _load_model(self):
        ...
        if not self.args.use_roberta:
            self.inference = ModelInference(self.colbert, amp=self.args.amp)
        else:
            print("encoder: robertaを使用")
            self.inference = ModelInference_X(self.colbert, amp=self.args.amp)
    ...
    def encode(self):
        ...
        for batch_idx, (offset, lines, owner) in enumerate(self._batch_passages(self.iterator)):
            ...
            t1 = time.time()
            batch = self._preprocess_batch(offset, lines) ##ここで文書をエンコード
            embs, doclens = self._encode_batch(batch_idx, batch) ##ここで文書をエンコードaaaaaaaaaaaaa
    ...
    def _preprocess_batch(self, offset, lines):
        ...
        for line_idx, line in zip(range(offset, endpos), lines):
            ...
            assert pid == 'id' or int(pid) == line_idx, f'{pid} {line_idx}'
