...
def train(args):
    ...
    if args.use_roberta:
            colbert = ColBERT_X.from_pretrained(args.model_name,
                                      ...
                                      ### BERTmodel.frompretrained Automodel
                                      ##モデルがわかっているのでColBERT.frompretrainedを使用？
    ...
    for batch_idx, BatchSteps in zip(range(start_batch_idx, args.maxsteps), reader):
        #print(batch_idx,(batch_idx - start_batch_idx) * args.bsize * args.nranks)
        this_batch_loss = 0.0

        for queries, passages in BatchSteps:
            count += 1
            if count == 1:
                with open("output_vector.txt","w")as fw:
                    fw.write(f'{queries}')
            with amp.context():
