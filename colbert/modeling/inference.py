...
from colbert.modeling.tokenization import QueryTokenizer, DocTokenizer, QueryTokenizer_X, DocTokenizer_X
...
class ModelInference():
    def __init__(self, colbert: ColBERT, amp=False):
        ...
        self.query_tokenizer = QueryTokenizer(colbert.query_maxlen,colbert.tok_name)
        self.doc_tokenizer = DocTokenizer(colbert.doc_maxlen,colbert.tok_name)
...
class ModelInference_X():
    def __init__(self, roberta: ColBERT_X, amp=False):
        assert roberta.training is False
        print("inference :roberta")
        self.roberta = roberta
        self.query_tokenizer = QueryTokenizer_X(roberta.query_maxlen,roberta.tok_name)
        self.doc_tokenizer = DocTokenizer_X(roberta.doc_maxlen,roberta.tok_name)
    ...
    def query(self, *args, to_cpu=False, **kw_args):
        with torch.no_grad():
            with self.amp_manager.context():
                Q = self.roberta.query(*args, **kw_args)
                return Q.cpu() if to_cpu else Q

    def doc(self, *args, to_cpu=False, **kw_args):
        with torch.no_grad():
            with self.amp_manager.context():
                D = self.roberta.doc(*args, **kw_args)
                return D.cpu() if to_cpu else D
    ...
    def queryFromText_X(self, queries, bsize=None,to_cpu=False,with_ids=False):
        if bsize:
            batches = self.query_tokenizer.tensorize(queries, bsize=bsize)
    ...
    def docFromText_X(self, docs, bsize=None, keep_dims=True, to_cpu=False, with_ids=False):
        if bsize:
